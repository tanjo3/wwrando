...
; In daItem_c::CreateInit, after mAction is stored from actor params, we detect our custom Blue ChuChu action marker (0x3E) and overwrite mAction to BOSS_DISAPPEAR (5) so that initAction() gives the item the correct grow+spin+ding spawn animation.
; The raw params at offset 0xB0 are NOT modified, so downstream hooks can still identify the item by checking params bits [31:26].
.org 0x800F51DC ; In daItem_c::CreateInit
...
; Death-path items have a custom m_itemNo (not 0x4B) but still use stage 0xE.
; We identify them by checking item_action == 0x3E from the raw actor params, which is a unique tag we set in blue_chuchu_create_death_item.
